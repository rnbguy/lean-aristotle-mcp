...
mcp: FastMCP[None] = FastMCP(
    ...
    instructions=(
        ...
        "to Lean. Use project lifecycle tools with explicit project IDs to inspect, cancel, "
        "or download artifacts for known Aristotle projects.\n\n"
        "IMPORTANT: Proofs generated by Aristotle require Mathlib tactics. Ensure the Lean "
        "project has Mathlib as a dependency and include `import Mathlib.Tactic` at the top "
        "of any file using Aristotle-generated proofs.\n\n"
...
async def formalize_tool(
    ...
    Note:
        Generated code requires `import Mathlib.Tactic` to run.
    """
...
async def get_project_tool(project_id: str) -> ResultDict:
    """Get metadata for a known Aristotle project.

    Use this tool when you already have a project_id from a previous Aristotle
    submission and want current status, timestamps, prompt, or output summary.

    Args:
        project_id: Aristotle project ID returned by another tool or the Aristotle CLI

    Returns:
        JSON with project status, raw Aristotle status, percent_complete,
        timestamps, prompt metadata, and any output summary.
    """
    result = await get_project(project_id=project_id)
    return result.to_dict()


@mcp.tool(name="cancel_project")
async def cancel_project_tool(project_id: str) -> ResultDict:
    """Cancel a queued or in-progress Aristotle project.

    Use this when a submitted job is no longer useful because the statement,
    context, or prompt was wrong.

    Args:
        project_id: Aristotle project ID to cancel

    Returns:
        JSON with the resulting project status.
    """
    result = await cancel_project(project_id=project_id)
    return result.to_dict()


@mcp.tool(name="get_solution")
async def get_solution_tool(
    project_id: str,
    output_path: str | None = None,
    overwrite: bool = False,
) -> ResultDict:
    """Download a completed Aristotle project's solution archive.

    Args:
        project_id: Aristotle project ID
        output_path: Optional local path for the downloaded .tar.gz archive.
                     If omitted, uses a unique {project_id}_aristotle.tar.gz path.
        overwrite: If True, allow replacing an existing output_path.

    Returns:
        JSON with status and output_path when the archive is saved.
    """
...
@mcp.tool(name="get_solution_if_complete")
async def get_solution_if_complete_tool(
    project_id: str,
    output_path: str | None = None,
    overwrite: bool = False,
) -> ResultDict:
    """Download a solution archive only if Aristotle has output available.

    This is the non-error polling-friendly variant of get_solution: queued and
    in-progress projects return their current status without writing a file.
    ...
    Returns:
        JSON with status and output_path when the archive is saved.
    """
    result = await get_solution_if_complete(
        project_id=project_id,
        output_path=output_path,
        overwrite=overwrite,
    )
...
@mcp.tool(name="get_input")
async def get_input_tool(
    project_id: str,
    output_path: str | None = None,
    overwrite: bool = False,
) -> ResultDict:
    """Download the original input archive for a known Aristotle project.

    Args:
        project_id: Aristotle project ID
        output_path: Optional local path for the downloaded .tar.gz archive.
                     If omitted, uses a unique {project_id}_input.tar.gz path.
        overwrite: If True, allow replacing an existing output_path.
